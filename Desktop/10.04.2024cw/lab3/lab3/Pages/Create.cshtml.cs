...
namespace lab3.Pages
{
    public class CreateModel : PageModel
    {
        ...
        public CreateModel(LibraryContext db)
        {
            context = db;
        }
        ...
        public async Task<IActionResult> OnPost(string book, string publisher, string author, string style, string year)
        {
            ...
            context.Books.Add(new Book()
            {
                Name = book,
                Year = int.Parse(year),
                PublisherId = context.Publishers.First(x => x.Name == publisher).Id,
                AuthorId = context.Authors.First(x => x.Name == author).Id,
                StyleId = context.Styles.First(x => x.Name == style).Id,
            });
